...
static unsigned int findShortestPath(int part) {
  ...
  std::vector<int> visited(map.width * map.height);
  int veryBest = -1;
...
static unsigned int part1() { return findShortestPath(1); }

static unsigned int part2() { return findShortestPath(2); }

static unsigned int part3() { return findShortestPath(3); }
} // namespace quest13_namespace

void quest13() {
  printf("QUEST %s\n", DAY);

  unsigned int d1 = quest13_namespace::part1();
  printf("\tPART 1 = %u\n", d1);

  unsigned int d2 = quest13_namespace::part2();
  printf("\tPART 2 = %u\n", d2);

  unsigned int d3 = quest13_namespace::part3();
  printf("\tPART 3 = %u\n", d3);
}
