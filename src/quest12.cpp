...
static TRank shootStar(TPoint star, TPoint rocket, int coef, TRank best) {
  ...
  if ((rocket.x + C) == rocket.y) {
    ...
    if (altitude > best.altitude ||
        (altitude == best.altitude && score < best.score)) {
  ...
  for (int f = maxTime; f > 0; f--) {
    ...
    for (int step = start;; step++) {
      ...
      if (from.x == target.x && from.y == target.y) {
        int score = f * coef;
        if (from.y > best.altitude ||
            (from.y == best.altitude && score < best.score)) {
...
static unsigned int part1() { return shootThemAll(1); }

static unsigned int part2() { return shootThemAll(2); }
...
void quest12() {
  ...
  unsigned int d1 = quest12_namespace::part1();
  printf("\tPART 1 = %u\n", d1);

  unsigned int d2 = quest12_namespace::part2();
  printf("\tPART 2 = %u\n", d2);

  unsigned int d3 = quest12_namespace::part3();
  printf("\tPART 3 = %u\n", d3);
}
