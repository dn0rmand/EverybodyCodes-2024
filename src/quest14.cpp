...
static bool isValid(const std::set<u_long> &points, const std::set<u_long> &visited, TPoint pt)
{
    u_long k = getKey(pt);
...
static void addPoint(std::map<int, int> &distances, std::set<u_long> *visited, std::vector<TPoint> *newStates, int steps, TPoint p)
{
    auto k = getKey(p);
    if (p.x == 0 && p.z == 0)
    {
...
static std::map<int, int> distanceToTrunk(const std::set<u_long> &points, u_long leafKey)
{
    ...
    while (states->size() > 0)
    {
        ...
        for (auto s : *states)
        {
            ...
            {
                addPoint(distances, &visited, newStates, steps, {.x = s.x, .y = s.y, .z = s.z - 1});
            }
            if (isValid(points, visited, {.x = s.x, .y = s.y, .z = s.z + 1}))
            {
                addPoint(distances, &visited, newStates, steps, {.x = s.x, .y = s.y, .z = s.z + 1});
            ...
            {
                addPoint(distances, &visited, newStates, steps, {.x = s.x, .y = s.y - 1, .z = s.z});
            }
            if (isValid(points, visited, {.x = s.x, .y = s.y + 1, .z = s.z}))
            {
                addPoint(distances, &visited, newStates, steps, {.x = s.x, .y = s.y + 1, .z = s.z});
            ...
            {
                addPoint(distances, &visited, newStates, steps, {.x = s.x - 1, .y = s.y, .z = s.z});
            }
            if (isValid(points, visited, {.x = s.x + 1, .y = s.y, .z = s.z}))
            {
                addPoint(distances, &visited, newStates, steps, {.x = s.x + 1, .y = s.y, .z = s.z});
