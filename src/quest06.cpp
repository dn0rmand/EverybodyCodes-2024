...
static const char *process(int part) {
  ...
  std::vector<std::list<int>> paths(appleId);
  int lastId = 0;
  for (int id = 1;; id++) {
    lastId = id;
    int parent = tree[-id];
...
static const char *part1() { return process(1); }

static const char *part2() { return process(2); }

static const char *part3() { return process(3); }
} // namespace quest06_namespace

void quest06() {
  printf("QUEST %s\n", DAY);

  const char *d1 = quest06_namespace::part1();
  printf("\tPART 1 = %s\n", d1);

  const char *d2 = quest06_namespace::part2();
  printf("\tPART 2 = %s\n", d2);

  const char *d3 = quest06_namespace::part3();
  printf("\tPART 3 = %s\n", d3);
}
