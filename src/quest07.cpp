...
public:
    ...
    {
        std::vector<int> *path = new std::vector<int>(this->path);
        return *path;
    }
...
static std::vector<int> &parse(std::map<char, std::vector<int>> &data, int part)
{
...
static void strategies(std::function<void(std::vector<int> &strategy)> callback)
{
...
static long getScore(std::vector<int> &track, std::vector<int> &actions, int rounds)
{
    long score = 0;
    long current = 10;
...
static char *process(int part, int rounds)
{
    ...
    players.sort([&](const char p1, const char p2)
