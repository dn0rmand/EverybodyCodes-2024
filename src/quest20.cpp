...
static bool addState(tools::Map &map, std::map<u_long, TState> &newStates, TState &state, int startHeight)
{
...
static int getMaxHeight(tools::Map &map, int time, int startHeight)
{
    ...
    for (int x = 0; x < map.width; x++)
    {
        if (map.get(x, 0) == 'S')
        {
            ...
            (*states)[2] = {.x = x, .y = 0, .direction = right, .height = startHeight};
            break;
        }
    }
    ...
    while (time--)
    {
        ...
        for (auto e : (*states))
        {
            ...
            for (auto newState : {move(state, state.direction), turnLeft(state), turnRight(state)})
            {
...
static int furthestPath(tools::Map &map, int startX, int startHeight, int column)
{
    int furthest = 0;
    int sums[map.height];
    int s = 0;
