...
static const char *words_1[] = {"AL", "DO", "LL",  "OL",
                                "OR", "SI", "LOR", NULL};

static const Word words_2[] = {
    {10, "HCZCBQPOTD"}, {10, "IHTUPFJPAF"}, {10, "KORKMSNXKM"},
    {10, "KRWGKPLWWS"}, {10, "LVPUKVXLEI"}, {10, "MDNLHOQIQA"},
    {10, "QLRGDOFAEQ"}, {10, "QWOGDEOMUG"}, {10, "SZURECCYEH"},
    {10, "WYWVYMCXRL"},

    {4, "APSM"},        {4, "CQZF"},        {4, "CVTY"},
    {4, "CYZO"},        {4, "DBJP"},        {4, "EIQM"},
    {4, "LVHB"},        {4, "MBOX"},        {4, "PLVF"},
    {4, "ZBIM"},
...
// Unused array - commented out to avoid warning
/*
static const Word words_3[] = {
    {10, "ADAUEEZVFU"}, {10, "DQBNEZVMGA"}, {10, "EPOWEGNPUH"},
    {10, "FSKCODNWYE"}, {10, "FVUODMKIZU"}, {10, "HAJOPCTXJI"},
    {10, "HMUHBWALXQ"}, {10, "IBUZVPBMJM"}, {10, "MPTYSCWNUT"},
    {10, "QLCORJHVQQ"}, {10, "QLUEECIPIN"}, {10, "QTQBCDRFTP"},
    {10, "RDHAERHYKX"}, {10, "TATUNIISAP"}, {10, "TSWOFFKWIS"},
    {10, "UGGNNIYJAS"}, {10, "WZPPHTMQKZ"}, {10, "XGRCDOMRTC"},
    {10, "XHXBADYWOE"}, {10, "YDXEQOKXQH"},

    {4, "BIFG"},        {4, "CLDD"},        {4, "CLRZ"},
    {4, "FAJU"},        {4, "FIOD"},        {4, "FTYX"},
    {4, "IBPF"},        {4, "OPNI"},        {4, "OYOW"},
    {4, "PAQN"},        {4, "RDOT"},        {4, "RNJW"},
    {4, "RYLC"},        {4, "WMWF"},        {4, "WUJL"},
    {4, "XLVI"},        {4, "YJQK"},        {4, "YORT"},
    {4, "ZBUZ"},        {4, "ZXOH"},
...
static unsigned int part2() {
  ...
  while (index < totalLength) {
    ...
    remaining = 0; // Reset remaining for each new sentence

    for (const char *ptr = str; *ptr >= 32; ptr++) {
      for (short i = 0; words_2[i].length > 0; i++) {
        if (remaining >= words_2[i].length) {
          continue; // Skip shorter words if we already have a longer match
        }

        if (matches(ptr, words_2[i].word) ||
            reverseMatches(ptr, words_2[i].word, words_2[i].length)) {
...
void quest02() {
  ...
  unsigned int d1 = quest02_namespace::part1();
  printf("\tPART 1 = %u\n", d1);

  unsigned int d2 = quest02_namespace::part2();
  printf("\tPART 2 = %u\n", d2);

  unsigned int d3 = quest02_namespace::part3();
  printf("\tPART 3 = %u\n", d3);
}
